...
class ListsSuite extends FunSuite {
  ...
  test("sum of no numbers should be 0") {
    assert(sum(List()) === 0)
  }
  
  test("sum of a single number should be that number") {
    assert(sum(List(3)) === 3)
  }
  ...
  test("max when there are negative numbers") {
    assert(max(List(-2, -4, -4, -10, -2)) === -2)
  }
  
   test("max when there are no numbers should throw an exception") {
    intercept[IllegalArgumentException] {
    	max(List())
    }
  }
